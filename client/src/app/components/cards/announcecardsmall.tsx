...
const AnnounceCardSmall: React.FC<AnnounceCardProps> = ({
  ...
  return (
    <div  onClick={onClick}
    className="bg-sky-500 text-black rounded-2xl p-6 flex gap-6 max-w-3xl shadow-md 
      ...
      <div className="flex flex-col space-y-2">
        {/* Category */}
        <span className="font-raleway bg-white text-primary3 text-sm font-medium px-3 py-1 rounded-full w-fit">
          {category}
        </span>
        ...
        {/* Date */}
        <span className="font-raleway text-sm text-white/90 mt-1">{date}</span>
      </div>
    </div>
  );
};
