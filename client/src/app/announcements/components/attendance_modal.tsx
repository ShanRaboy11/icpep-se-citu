    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-[#003599] to-[#0055cc] text-white p-8 flex justify-between items-start">
          <div>
            <h2 className="text-3xl font-bold mb-2">Complete Meeting Attendance</h2>
            <p className="text-blue-200 text-lg">ICpEP.SE CIT-U Chapter - Quarterly Board Meeting</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-8 overflow-y-auto max-h-[calc(90vh-200px)] space-y-6">
          ...
          <div className="bg-[#003599]/5 rounded-xl p-4">
            <h3 className="text-base font-semibold text-[#003599] mb-3 flex items-center">
            ...
            <div className="space-y-2">
              {councilOfficers.map((officer, index) => (
                <div 
                  key={index} 
                  className="flex items-center justify-between p-2.5 bg-white rounded-lg shadow-sm border-l-4 border-[#003599]"
                >
                  <span className="font-medium text-[#003599] text-sm min-w-[90px]">{officer.title}</span>
                  <span className="text-gray-700 text-right text-sm">{officer.name}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {committees.map((committee, idx) => (
            <div key={idx} className={`${committee.bgColor} rounded-xl p-4`}>
              <h3 className="text-base font-semibold mb-3 flex items-center" style={{ color: committee.color }}>
                <span 
              ...
              <div className="space-y-2">
                {committee.members.map((member, index) => (
                  <div 
                    key={index} 
                    className="flex items-center justify-between p-2.5 bg-white rounded-lg shadow-sm border-l-4"
                    style={{ borderLeftColor: committee.color }}
                  >
                    <span className="font-medium text-sm min-w-[90px]" style={{ color: committee.color }}>
                      {member.title}
                    </span>
                    <span className="text-gray-700 text-right text-sm">{member.name}</span>
                  </div>
                ))}
              </div>
            </div>
