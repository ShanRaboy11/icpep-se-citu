...
import { Box, Lightbulb, Send, Ribbon } from "lucide-react";
...
const InfoSection: FC = () => {
  ...
  useEffect(() => {
    // Clear and restart interval whenever activeIndex changes
    const interval = setInterval(() => {
      setActiveIndex((prevIndex) => (prevIndex + 1) % sections.length);
    }, TAB_DURATION);

    return () => clearInterval(interval); // cleanup
  }, [activeIndex]); // dependency ensures restart on tab click
  ...
  return (
    ...
    px-10 sm:px-16 py-16 sm:py-20 shadow-2xl text-white"
      >
        {/* Animate only the contents */}
        <div
          key={activeIndex}
          className="content-fade grid md:grid-cols-2 gap-8 md:gap-12 items-start"
        >
          {/* Text side */}
          <div className="text-left">
            ...
            <p className="font-raleway text-lg sm:text-xl leading-relaxed text-gray-100">
